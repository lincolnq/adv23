...
m = Matrix().parse(input)
print(m)

### This whole section is based on SO:
# https://stackoverflow.com/questions/35610834/find-consecutive-ones-in-numpy-array?rq=3
append1 = np.zeros((m.shape[0],1),dtype=bool)
digits_bool = np.hstack((append1, np.char.isdigit(m), append1))
print(digits_bool)

run_starters = np.ones_like(digits_bool)
#np.bitwise_xor(digits_bool[:, :-1], digits_bool[:, 1:], out=run_starters[:, 1:])  # set equal, consecutive elements to 0
#print(run_starters)
diffs = np.diff(digits_bool.astype(int),axis=1)
print(diffs)
#diffs = diffs * digits_bool[:, :-1]
#print(diffs)
starts = np.argwhere(diffs == 1)
stops = np.argwhere(diffs == -1)
start_stop = np.column_stack((starts[:, 0], starts[:, 1], (stops - starts)[:, 1]))
print(start_stop)

# Start_stop is now an Nx3 array where each row is a sequence of digits
# (potential "part number"). Columns are: 
# row number in m, column in m where partno starts, length of partno

# need to check for gears. Just grab a subview of m
# in the neighborhood of each potential partno and look for gears

# a gear will be '(row, col) -> [partnos]'
from collections import defaultdict
gears = defaultdict(lambda: [])

for part in start_stop:
    (r, c, plen) = part
    partno = m[r, c:c+plen]
    rstart = max(0, r-1)
    cstart = max(0, c-1)
    neighborhood = m[rstart:r+2, cstart:c+plen+1]
    #print(part)
    #print(neighborhood)
    gearsqs = np.argwhere(neighborhood == '*')
    for g in gearsqs:
        gprime = g + np.array([rstart, cstart])
        gpt = tuple(gprime)
        #print(repr(gpt))
        gears[gpt].append(''.join(partno))
