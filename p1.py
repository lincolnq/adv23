...
digitstrs = r"one|two|three|four|five|six|seven|eight|nine|1|2|3|4|5|6|7|8|9".split("|")
digits = {s: str(i) for s,i in zip(digitstrs, range(1,10))} | {str(i): str(i) for i in range(10)}
...
def test(s: str):
    ...
    firstdigit = min(digitstrs, key=key)
    lastdigit = max(digitstrs, key=lambda d: s.rfind(d))
    #m = re.findall(digitstrs, s)
    ccnum = int(digits[firstdigit] + digits[lastdigit])
    return ccnum

if __name__ == "__main__":
    tot = 0
    # concatenate first and last instances of any digit in each row
    for row in input:
        # use re to find first of any digit
        ccnum = test(row)
        print(ccnum)
        tot += ccnum
